...
class TestApp(EClient, EWrapper):
    ...
    def start(self):
        ...
        # Order Type: Market To Limit Order
        # Explanation: Market order to execute at the current best market price. 
        # If only partially filled, remainder of the order canceled and re-submitted as a limit order with the 
        # limit price equal to the price at which the filled portion of the order executed.
        # General Link: https://www.interactivebrokers.com/en/trading/ordertypes.php?m=markettoLimitModal
        # API Link: https://www.interactivebrokers.com/campus/ibkr-api-page/order-types/#market-to-limit-order
        # Building the Order with required attributes
        order = Order()
        order.action = "BUY"  # Specifies whether the order is to buy or sell (e.g., 'BUY', 'SELL')
        order.orderType = "MTL"  # Type of order such as Market, Limit, Stop, etc.
